...
function extractMetadataFromPath(
  ...
  let teacherName = 'All Teachers'; // New field
  let category = 'Uncategorized';
  const tags: string[] = [];

  // Path segments are ordered from top-level (e.g., Year) down to the direct parent of the file (e.g., Category)
  // Expected structure: Year/Semester/CourseName/TeacherName/Category/file.pdf
  // Or for Job Prep: Job Preparation/SubFolderAsTeacherOrCat/SubSubFolderAsCat/file.pdf
  ...
  if (folderPathSegments.length > 0) {
    ...
    if (firstSegmentLower.includes('1st year')) year = '1st Year';
      ...
      if (folderPathSegments.length > 1) {
        // If segment 2 looks like a common category, assume segment 1 is not a teacher
        ...
        if (commonCategories.some(cat => seg2Lower.includes(cat))) {
            ...
            teacherName = 'All Teachers'; // Or 'General'
        } else {
            teacherName = folderPathSegments[1]; // Assume it's a teacher/sub-type
      ...
      // Early partial return or completion for job prep.
      // We'll let the main logic add filename tags later.
    }
  ...
  if (!courseName.includes('Job Preparation')) {
    ...
    // New logic for Teacher and Category
    // Year/Semester/CourseName/TeacherName/Category/file.pdf (5 segments to category)
    // Year/Semester/CourseName/Category/file.pdf (4 segments to category, no explicit teacher)
    if (folderPathSegments.length > 3) {
        // Check if folderPathSegments[3] is likely a category or a teacher name
        const segment3Lower = folderPathSegments[3].toLowerCase();
        const commonCategories = ['lecture notes', 'past paper', 'lab manual', 'book', 'notes', 'slide', 'presentation', 'question'];

        if (folderPathSegments.length > 4 && !commonCategories.some(cat => segment3Lower.includes(cat))) { 
            // Likely Year/Sem/Course/Teacher/Category
            teacherName = folderPathSegments[3];
            category = folderPathSegments[4];
            if (teacherName !== 'All Teachers') tags.push(teacherName.toLowerCase().replace(/\s+/g, '-'));
        } else {
            // Likely Year/Sem/Course/Category (no explicit teacher folder or teacher folder is also a category)
  ...
  // Refined category mapping (can be applied after potential teacher name)
  ...
  // No else, keep original category if not matched, or if it was 'Job Preparation' or 'Uncategorized'
...
export async function GET() {
  ...
  try {
    ...
    if (resources.length === 0 && driveFiles.length > 0) {
        console.warn(`No resources processed. This might be due to GOOGLE_DRIVE_FOLDER_ID ('${GOOGLE_DRIVE_FOLDER_ID}') not being an ancestor of any files, or the folder structure doesn't match expectations for path reconstruction. Path segments example: ${JSON.stringify(driveFiles[0]?.parents)}`);
    }
