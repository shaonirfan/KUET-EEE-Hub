...
// Base static categories are not strictly needed if dynamic ones are always preferred
...
export default function ResourcesSection() {
  ...
  const [selectedCategory, setSelectedCategory] = useState<string>(''); // Will be set by useEffect
  ...
  useEffect(() => {
    const fetchResources = async () => {
      ...
      try {
        ...
        setAllResources([]); // Ensure allResources is empty on error
      } finally {
        setIsLoading(false);
      }
    };
  ...
  }, [allResources, selectedYear, selectedSemester, isLoading]);
  ...
  const dynamicTeacherNames = useMemo(() => {
    ...
    return ['All Teachers', ...Array.from(teachers).sort().filter(tn => tn.toLowerCase() !== 'all teachers')];
  }, [allResources, selectedYear, selectedSemester, selectedCourseName, isLoading]);
  
  const dynamicCategories = useMemo(() => {
    if (isLoading || !selectedYear || !selectedSemester) return [];
  ...
  }, [allResources, selectedYear, selectedSemester, selectedCourseName, selectedTeacherName, isLoading]);


  useEffect(() => {
    if (!isLoading && allResources.length > 0) {
      setSelectedCourseName('All Courses');
      // Teacher and Category will be reset by subsequent effects
    }
  }, [selectedYear, selectedSemester, allResources, isLoading]);

  useEffect(() => {
     if (!isLoading && allResources.length > 0) {
      setSelectedTeacherName('All Teachers');
      // Category will be reset by subsequent effect
    }
  }, [selectedCourseName, selectedYear, selectedSemester, allResources, isLoading]);

  useEffect(() => {
    if (!isLoading && allResources.length > 0) {
      if (dynamicCategories.length > 0) {
        if (!dynamicCategories.includes(selectedCategory) || selectedCategory === '') {
  ...
  const resetFilters = () => {
    ...
    // setSelectedCourseName('All Courses'); // Will be reset by useEffect
    // setSelectedTeacherName('All Teachers'); // Will be reset by useEffect
    // setSelectedCategory will be reset by its useEffect based on dynamicCategories
  };
  ...
  const activeFiltersCount = useMemo(() => {
    ...
    // Category is always "active" with a specific selection,
    // so count it if it's not the first dynamically available one (if more than one exists)
    if (dynamicCategories.length > 0 && selectedCategory !== dynamicCategories[0]) count++;
    else if (dynamicCategories.length === 0 && selectedCategory !== '') count++; // if no categories, but one was somehow selected
    return count;
  }, [searchTerm, selectedYear, selectedSemester, selectedCourseName, selectedTeacherName, selectedCategory, dynamicCategories]);
    ...
    <section id="resources" className="container mx-auto px-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-6">
              ...
              <div>
               ...
               <div>
                ...
                >
                  ...
                  <SelectContent>
                     {/* Ensure "All Teachers" is always first if other teachers exist or it's the only option */}
                    {(dynamicTeacherNames.length > 1 || (dynamicTeacherNames.length === 1 && dynamicTeacherNames[0] !== 'All Teachers')) && (
                         <SelectItem key="All Teachers" value="All Teachers">All Teachers</SelectItem>
                    )}
                    {dynamicTeacherNames.filter(teacher => teacher !== 'All Teachers').map(teacher => (
                      <SelectItem key={teacher} value={teacher}>{teacher}</SelectItem>
                    ))}
                     {dynamicTeacherNames.length === 1 && dynamicTeacherNames[0] === 'All Teachers' && (
                        <SelectItem key="All Teachers" value="All Teachers">All Teachers</SelectItem>
                     )}
                  </SelectContent>
                </Select>
              ...
              <div className="md:col-span-2">
                ...
                {isLoading ? (
                  ...
                  <Tabs value={selectedCategory} onValueChange={setSelectedCategory} defaultValue={dynamicCategories[0]}>
      ...
      {!isLoading && !error && filteredResources.length === 0 && allResources.length > 0 && (
         <div className="text-center py-16">
          ...
          <p className="text-md text-muted-foreground mt-2">Try adjusting your search or selections. Ensure year, semester, and category are selected.</p>
        </div>
      )}

      {!isLoading && !error && allResources.length === 0 && !isLoading && ( // Added !isLoading here too
         <div className="text-center py-16">
